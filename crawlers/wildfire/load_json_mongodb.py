...
async def update_or_insert_earthquakes_async(records, collection):
    bulk_operations = [
        UpdateOne(
            {"properties.UniqueId": record["properties"]["UniqueId"]},
            {"$set": record},
            upsert=True,
        )
...
async def main():
    ...
    with open("./fires_ca.json", "r") as file:
        data = json.load(file)

    client = connect_to_mongodb()

    db = client["ENDDB"]
    collection = db["wildfires"]  # Replace with your actual collection name

    # Use 'id' as '_id' in MongoDB for each feature
    for feature in data["features"]:
        feature["_id"] = feature["properties"]["UniqueId"]
